export class DataStore {
  ...
  // Append output into an array under user -> testcase -> scenario -> key
  // If key doesn't exist => create []
  // If key exists as non-array => convert to [oldValue]
  // Then push new value
  appendScenarioOutputArray(userId, testCaseId, scenarioId, key, value) {
    this.ensureContainer(userId, testCaseId, scenarioId);

    const container = this.userOutputs[userId][testCaseId][scenarioId];
    const existing = container[key];

    if (existing === undefined) {
      container[key] = [value];
      console.log("array ", container[key]);
      return;
    }

    if (Array.isArray(existing)) {
      existing.push(value);
      console.log("array ", existing);
      return;
    }


    // existing is a single value (string/number/object) -> convert to array
